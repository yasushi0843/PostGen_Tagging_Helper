...
now = time.time()
dirs = ([os.path.abspath(os.path.join(args.dir, e)) for e in os.listdir(args.dir)
            if os.path.isdir(os.path.join(args.dir, e))
            and (args.prefix is None or e.startswith(args.prefix))
            and (args.suffix is None or e.endswith(args.suffix))
            and (args.exclude_prefix is None or not e.startswith(args.exclude_prefix))
            and (args.exclude_suffix is None or not e.endswith(args.exclude_suffix))]
            if args.sub else [os.path.abspath(args.dir)])
